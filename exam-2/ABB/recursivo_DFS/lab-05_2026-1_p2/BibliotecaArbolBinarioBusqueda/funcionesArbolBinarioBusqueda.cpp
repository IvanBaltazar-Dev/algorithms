...
void insertarRecursivo(struct NodoArbolBinarioBusqueda *&raiz, const struct ElementoArbolBinarioBusqueda &elemento) {
    if (esNodoVacio(raiz)) { //EL NODO ESTA VACIO INICIALIZA
        plantarNodoArbolBinario(raiz, nullptr, elemento, nullptr);
        raiz->elemento.cantidad = 1;
    } else {
        if (raiz->elemento.palabra > elemento.palabra) {
            insertarRecursivo(raiz->izquierda, elemento);
        } else {
            if (raiz->elemento.palabra < elemento.palabra) {
                insertarRecursivo(raiz->derecha, elemento);
            } else { //SI LA PALABRA ES IGUAL AUMENTO CANTIDAD
                // cout << "Cantidad antes: " << raiz->elemento.cantidad << endl;
                raiz->elemento.cantidad++;
                // cout << "El elemento \"" << elemento.palabra << "\". Ya se encuentra en el arbol" << endl;
                // cout << "Cantidad despues: " << raiz->elemento.cantidad << endl;
                // cout << endl << endl;
            }
        }
    }
...
int comparaElementos(const string &elementoA, const string &elementoB){
...
bool buscarRecursivo(struct NodoArbolBinarioBusqueda * nodo, const struct ElementoArbolBinarioBusqueda & elemento){
    if(esNodoVacio(nodo))
        return false;
    if(comparaElementos(nodo->elemento.palabra, elemento.palabra) == 0)
        return true;
    if(comparaElementos(nodo->elemento.palabra, elemento.palabra) == 1)
...
struct NodoArbolBinarioBusqueda * eliminarRecursivo(struct NodoArbolBinarioBusqueda * nodo,
    ...
    if(nodo->elemento.palabra > elemento.palabra)
...
void imprimirNodo(struct NodoArbolBinarioBusqueda *raiz) {
    cout << "          "<< left <<setw(15) << raiz->elemento.palabra << " |   "<<raiz->elemento.cantidad << "  ";
    cout<<endl;;
}
